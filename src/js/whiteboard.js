...
const whiteboard = {
  ...
  triggerMouseMove: function (e) {
    ...
    window.requestAnimationFrame(function () {
      ...
      if (_this.tool === "eraser") {
        ...
        _this.tool === "rect" ||
        (_this.tool === "recSelect" && _this.drawFlag)
      ) {
        if (_this.svgRect) {
  ...
  redrawMouseCursor: function () {
    ...
    _this.triggerMouseMove({
      offsetX: _this.prevPos.x,
      offsetY: _this.prevPos.y,
    });
  },
  ...
  setTextBackgroundColor(textboxBackgroundColor) {
    ...
    $("#textboxBackgroundColorPicker").css({
    ...
    if (
      ...
      _this.setTextboxBackgroundColor(
        _this.latestActiveTextBoxId,
        textboxBackgroundColor
      );
    }
    ...
    window.requestAnimationFrame(function () {
      if (tool === "line" || tool === "pen") {
        ...
        if (content["event"] === "move") {
          if (
            _this.cursorContainer.find("." + content["username"]).length >= 1
          ) {
            _this.cursorContainer
